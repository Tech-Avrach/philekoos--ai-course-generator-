...
const getChapterDetailPrompt = (courseName, chapter) => {
    return `Explain the concept in detail on Topic: ${courseName}, Chapter: ${chapter}. Provide the response strictly in JSON format...`;
};

const getCorrectJsonPrompt = (contentText) => {
    return `Given the following JSON data, identify and correct any syntax issues... JSON data to correct: ${contentText}`;
};
...
const getVideoUrlId = async (courseName, chapter) => {
    ...
    try {
        const videoResponse = await service.getVideos(`${chapter}`);

        console.log("videoResponse", videoResponse)

        return videoResponse[0]?.id?.videoId || '';
    } catch (error) {
        handleError('Failed to get video URL', error);
        return '';
    }
};
...
function CourseBasicInfo({ course, GetCourse }) {
    ...
    }, [courseImageFile, course, GetCourse]);

    // Function to handle chapter content generation
    const generateChapterContent = useCallback(async (courseName, chapterName, index) => {
        let prompt = getChapterDetailPrompt(courseName, chapterName);
        let contentResult;
        let contentText;
        ...
        try {
            ...
            try {
                contentResult = await useMistralAi(prompt);
                if (contentResult.startsWith("```json") && contentResult.endsWith("```")) {
                    contentResult = contentResult.slice(7, -3).trim();
                }
                contentText = contentResult;
            } catch (error) {
    ...
    // Function to validate and correct JSON
    const validateAndCorrectJson = useCallback(async (contentText) => {
        let content;

        try {
            content = JSON.parse(contentText);
        } catch {
            let correctJsonPrompt = getCorrectJsonPrompt(contentText);
            try {
                const CorrectJsonResult = await CorrectJsonFormat_AI.sendMessage(correctJsonPrompt);
                content = JSON.parse(CorrectJsonResult.response?.text());
            } catch {
                try {
                    const result = await useMistralAi(correctJsonPrompt);
                    if (result.startsWith("```json") && result.endsWith("```")) {
                        content = JSON.parse(result.slice(7, -3).trim());
                    }
                } catch {
    ...
    // Function to handle the course generation process
    const handleStartCourse = useCallback(async () => {
        if (!courseImageFile && !courseImage) {
            toast.error("Please select an image", { className: "border border-primary" });
            return;
        }
        ...
        for (const [index, chapter] of chapters.entries()) {
            try {
                const courseName = course?.courseOutput?.name;
                const chapterName = chapter?.name;
                let editedChapters = [...chaptersDetailsToGenerate];
                editedChapters[index].status = "progress";
                setChaptersDetailsToGenerate(editedChapters);

                let contentText = await generateChapterContent(courseName, chapterName, index);

                console.log("contentText", contentText);

                let content = await validateAndCorrectJson(contentText);

                console.log("content", content);


                let videoId = await getVideoUrlId(courseName, chapterName); // Add getVideoUrlId logic

                console.log("videoId", videoId);

                chapterInfoArray.push({
                    chapterId: index,
                    name: chapter?.name,
                    content,
                    videoId,
                ...
                editedChapters[index].status = "completed";
                setChaptersDetailsToGenerate(editedChapters);
            } catch (error) {
        ...
        if (!errorOccurred) {
            try {
                ...
                toast.success("Course content generated successfully", { className: "border border-primary" });
                router.replace(`/create-course/${course?.courseId}/finish`);
            } catch (error) {
                toast.error("Failed to save course content.", { className: "border border-primary" });
            }
        }

        setLoading(false);
    }, [course, courseImageFile, chaptersDetailsToGenerate]);
    ...
    const onFileChange = (event) => {
        ...
        if (file && file.type.startsWith("image/")) {
            ...
            toast.error("Please select a valid image file", { className: "border border-primary" });
        }
    };
    ...
    return (
        <div className="p-10 border rounded-xl shadow-sm mt-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="relative">
                    <label htmlFor="upload-image" className="group">
                        <div className="relative h-full w-full cursor-copy">
                            <Image
                                src={courseImage || '/courseImagePlaceholder.png'}
                                width={350}
                                height={350}
                                className="rounded-xl w-full object-cover h-full transition duration-300 ease-in-out group-hover:brightness-50"
                                alt="course image"
                            />
                        </div>
                    </label>
                    ...
                    />
                    <div className='absolute  bottom-5 lg:bottom-3 md:bottom-3 left-3 cursor-pointer flex'>
                        <div className='peer'>
                            <GenerateImage
                                imagePrompt={course?.courseOutput?.imagePrompt}
                                name={course?.name}
